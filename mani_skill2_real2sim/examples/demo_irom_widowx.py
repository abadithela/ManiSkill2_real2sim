...
def main():
    ...
    if (not hasattr(env, "prepackaged_config")) or (not env.prepackaged_config):
        ...
        if names_in_env_id_fxn(
            ...
            # TODO: Check initial positions
            elif env.robot_uid == "irom_widowx":
                real_robot_wrt_llc = [0.28, 0.225] # Real robot base position with respect to lower left corner. 
    ...
    EE_ACTION = (
        0.1 if not is_widowx else 0.02
    )  # widowx use unnormalized action space
    EE_ROT_ACTION = (
        1.0 if not is_widowx else 0.1
    )  # widowx use unnormalized action space
